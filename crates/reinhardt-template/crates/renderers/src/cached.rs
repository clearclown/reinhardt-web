...
mod tests {
    ...
    async fn test_cache_miss_different_context() {
        ...
        cached_renderer.render(&data, Some(&context1)).await.unwrap();
        assert_eq!(counter.load(Ordering::SeqCst), 1);

        cached_renderer.render(&data, Some(&context2)).await.unwrap();
        assert_eq!(counter.load(Ordering::SeqCst), 2);
    }
    ...
    async fn test_entry_count() {
        let cached_renderer =
            CachedRenderer::new(JSONRenderer::new(), CacheConfig::default());
    ...
    async fn test_renderer_trait_methods() {
        ...
        assert_eq!(cached_renderer.media_type(), "application/json; charset=utf-8");

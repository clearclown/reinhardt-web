...
impl Attachment {
    ...
    /// ```
    pub fn new(filename: impl Into<String>, content_type: impl Into<String>, content: Vec<u8>) -> Self {
        Self {
...
impl Email {
    ...
    pub fn validate(&self) -> EmailResult<()> {
        if self.from.is_empty() {
            return Err(EmailError::Validation("From address is required".to_string()));
        }
        if self.to.is_empty() {
            return Err(EmailError::Validation("At least one recipient is required".to_string()));
        }
...
impl EmailBuilder {
    ...
    pub fn build(self) -> Email {
        Email {
            ...
            to: self.to,
            cc: if self.cc.is_empty() { None } else { Some(self.cc) },
            bcc: if self.bcc.is_empty() { None } else { Some(self.bcc) },

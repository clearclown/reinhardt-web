...
impl PriorityTaskQueue {
    ...
    pub async fn dequeue(&self) -> TaskResult<Option<Box<dyn Task>>> {
        ...
        let selected_priority = self.select_priority_weighted(&priorities_with_weight, total_weight);
    ...
    fn select_priority_weighted(&self, priorities: &[(Priority, u32)], total_weight: u32) -> Priority {
        ...
        priorities.first().map(|(p, _)| *p).unwrap_or(Priority::Normal)
    }
...
mod tests {
    ...
    async fn test_priority_ordering() {
        ...
        queue.enqueue(Box::new(TestTask::new("low1")), Priority::Low).await.unwrap();
        queue.enqueue(Box::new(TestTask::new("high1")), Priority::High).await.unwrap();
        queue.enqueue(Box::new(TestTask::new("normal1")), Priority::Normal).await.unwrap();
        queue.enqueue(Box::new(TestTask::new("high2")), Priority::High).await.unwrap();
    ...
    async fn test_weighted_scheduling() {
        ...
        for i in 0..30 {
            queue.enqueue(Box::new(TestTask::new(&format!("high{}", i))), Priority::High).await.unwrap();
        }
        for i in 0..10 {
            queue.enqueue(Box::new(TestTask::new(&format!("normal{}", i))), Priority::Normal).await.unwrap();
        }
        for i in 0..5 {
            queue.enqueue(Box::new(TestTask::new(&format!("low{}", i))), Priority::Low).await.unwrap();
        }
    ...
    async fn test_fifo_within_priority() {
        ...
        queue.enqueue(Box::new(TestTask::new("task1")), Priority::Normal).await.unwrap();
        queue.enqueue(Box::new(TestTask::new("task2")), Priority::Normal).await.unwrap();
        queue.enqueue(Box::new(TestTask::new("task3")), Priority::Normal).await.unwrap();
    ...
    async fn test_concurrent_access() {
        ...
        for i in 0..10 {
            ...
            handles.push(tokio::spawn(async move {
                queue_clone.enqueue(Box::new(TestTask::new(&format!("task{}", i))), Priority::Normal).await.unwrap();
            }));
        }
    ...
    async fn test_custom_priority() {
        ...
        queue.enqueue(Box::new(TestTask::new("custom75")), Priority::Custom(75)).await.unwrap();
        queue.enqueue(Box::new(TestTask::new("high")), Priority::High).await.unwrap();
        queue.enqueue(Box::new(TestTask::new("normal")), Priority::Normal).await.unwrap();

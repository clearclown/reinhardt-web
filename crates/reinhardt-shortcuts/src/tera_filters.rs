...
impl Filter for TruncateCharsFilter {
    fn filter(&self, value: &Value, args: &HashMap<String, Value>) -> TeraResult<Value> {
        ...
        let length = args
            .get("length")
            .and_then(|v| v.as_u64())
            .ok_or_else(|| tera::Error::msg("Filter `truncate_chars` requires `length` argument"))?
            as usize;
        ...
        if s.len() <= length {
            ...
            let truncated = s.chars().take(length.saturating_sub(suffix.len())).collect::<String>();
...
impl Filter for AddClassFilter {
    fn filter(&self, value: &Value, args: &HashMap<String, Value>) -> TeraResult<Value> {
        ...
        if html.contains("class=\"") {
            Ok(Value::String(html.replace("class=\"", &format!("class=\"{} ", class))))
        } else if html.contains("<input") || html.contains("<select") || html.contains("<textarea") {
            Ok(Value::String(html.replace(">", &format!(" class=\"{}\">", class))))
        } else {
...
mod tests {
    ...
    fn test_truncate_chars_filter() {
        ...
        let result = filter.filter(&json!("This is a very long text"), &args).unwrap();
        assert_eq!(result, json!("This is..."));

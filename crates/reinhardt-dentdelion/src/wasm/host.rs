...
impl crate::wasm::runtime::reinhardt::dentdelion::host::Host for HostState {
	...
	fn get_config(&mut self, key: String) -> Option<Vec<u8>> {
		// Get config value and serialize to MessagePack
		Self::get_config(self, &key).and_then(|v| rmp_serde::to_vec(&v).ok())
	}

	fn set_config(&mut self, key: String, value: Vec<u8>) -> Result<(), GeneratedPluginError> {
		// Deserialize MessagePack to ConfigValue
		let config_value: ConfigValue = rmp_serde::from_slice(&value).map_err(|e| {
			to_generated_error(WitPluginError::with_details(
				400,
		...
		Self::set_config(self, &key, config_value)
	...
	fn register_service(
		...
		name: String,
		data: Vec<u8>,
	) -> Result<(), GeneratedPluginError> {
		Self::register_service(self, &name, data)
			.map_err(|e| to_generated_error(WitPluginError::from_plugin_error(&e)))
	}

	fn get_service(&mut self, name: String) -> Option<Vec<u8>> {
		Self::get_service(self, &name)
	}

	fn unregister_service(&mut self, name: String) -> Result<(), GeneratedPluginError> {
		Self::unregister_service(self, &name)
			.map_err(|e| to_generated_error(WitPluginError::from_plugin_error(&e)))
	}
	...
	) -> Result<GeneratedHttpResponse, GeneratedPluginError> {
		// Bridge async function to sync by blocking on current runtime
		tokio::runtime::Handle::current()
			.block_on(Self::http_get(self, &url, &headers))
	...
	fn http_post(
		...
		// Bridge async function to sync by blocking on current runtime
		tokio::runtime::Handle::current()
			.block_on(Self::http_post(self, &url, &body, &headers))
			.map(to_generated_http_response)
			.map_err(to_generated_error)
	}
	...
	fn db_query(&mut self, sql: String, params: Vec<u8>) -> Result<Vec<u8>, GeneratedPluginError> {
		// Bridge async function to sync by blocking on current runtime
		tokio::runtime::Handle::current()
			.block_on(Self::db_query(self, &sql, &params))
			.map_err(to_generated_error)
	}

	fn db_execute(&mut self, sql: String, params: Vec<u8>) -> Result<u64, GeneratedPluginError> {
		// Bridge async function to sync by blocking on current runtime
		tokio::runtime::Handle::current()
			.block_on(Self::db_execute(self, &sql, &params))
			.map_err(to_generated_error)
	}
}

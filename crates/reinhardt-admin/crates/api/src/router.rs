...
///
/// ```rust,no_run
/// use reinhardt_panel_api::{AdminSite, admin_routes};
///
...
pub fn admin_routes() -> UnifiedRouter {
	UnifiedRouter::new()
		...
		// Dashboard and favicon
		.endpoint(crate::handlers::dashboard)
		.endpoint(crate::handlers::favicon)
		// CRUD endpoints (automatically registered for all models via path parameters)
...
impl AdminRouter {
	...
	///
	/// ```rust,no_run
	/// use reinhardt_panel_api::{AdminSite, AdminRouter};
	/// use std::sync::Arc;
	///

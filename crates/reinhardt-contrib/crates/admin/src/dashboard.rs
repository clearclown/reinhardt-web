...
pub struct ChartWidget {
    ...
    data_fn: Arc<
        dyn Fn() -> futures::future::BoxFuture<'static, AdminResult<ChartData>> + Send + Sync,
    >,
}
...
mod tests {
    ...
    async fn test_widget_registry_get_by_position() {
        ...
        let config1 =
            WidgetConfig::new("links1", WidgetPosition::TopLeft).with_order(2);

        let widget2 = Arc::new(QuickLinksWidget::new("Links2", WidgetPosition::TopLeft));
        let config2 =
            WidgetConfig::new("links2", WidgetPosition::TopLeft).with_order(1);
    ...
    async fn test_quick_link_with_icon() {
        let link = QuickLink::new("Dashboard", "/dashboard")
            .with_icon("fa fa-dashboard");
    ...
    async fn test_stat_widget_render() {
        let widget = StatWidget::new("Total Users", WidgetPosition::TopLeft, || async {
            Ok(42)
        });
    ...
    async fn test_table_widget_render() {
        let widget = TableWidget::new(
            "Recent Orders",
            WidgetPosition::Center,
            vec!["Order ID".to_string(), "Customer".to_string(), "Total".to_string()],
            || async {
                Ok(vec![

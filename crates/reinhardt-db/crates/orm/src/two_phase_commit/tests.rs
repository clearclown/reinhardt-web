...
mod integration_tests {
    ...
    use crate::two_phase_commit::core::{
        TwoPhaseCoordinator, TwoPhaseError, TransactionState, MockParticipant,
    };
    ...
    async fn test_recovery_from_log() {
        ...
        coordinator1.add_participant(Box::new(MockParticipant::new("db1"))).await.unwrap();
        ...
        coordinator2.add_participant(Box::new(MockParticipant::new("db2"))).await.unwrap();
    ...
    async fn test_multiple_coordinators_isolation() {
        ...
        coord1.add_participant(Box::new(MockParticipant::new("db1"))).await.unwrap();
        coord2.add_participant(Box::new(MockParticipant::new("db2"))).await.unwrap();

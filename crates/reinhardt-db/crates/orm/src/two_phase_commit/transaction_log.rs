...
use super::{TwoPhaseError, TransactionState};
...
pub trait TransactionLog: Send + Sync {
    ...
    fn find_by_state(&self, state: TransactionState)
    ...
    pub fn new(log_dir: PathBuf) -> Result<Self, TwoPhaseError> {
        ...
        if !log_dir.exists() {
            std::fs::create_dir_all(&log_dir)
                .map_err(|e| TwoPhaseError::LogError(format!("Failed to create log directory: {}", e)))?;
        }
...
impl TransactionLog for FileTransactionLog {
    ...
    fn read_all(&self) -> Result<Vec<TransactionLogEntry>, TwoPhaseError> {
        ...
        for entry_result in dir_entries {
            let entry = entry_result
                .map_err(|e| TwoPhaseError::LogError(format!("Failed to read directory entry: {}", e)))?;
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                ...
                let log_entry: TransactionLogEntry = serde_json::from_str(&json)
                    .map_err(|e| TwoPhaseError::LogError(format!("Deserialization error: {}", e)))?;
    ...
    fn delete(&self, transaction_id: &str) -> Result<(), TwoPhaseError> {
        ...
        if path.exists() {
            std::fs::remove_file(&path)
                .map_err(|e| TwoPhaseError::LogError(format!("Failed to delete log file: {}", e)))?;
        }
    ...
    fn find_by_state(
        ...
        Ok(all_entries.into_iter().filter(|e| e.state == state).collect())
    }
}
...
mod tests {
    ...
    fn test_log_entry_with_metadata() {
        ...
        assert_eq!(entry.metadata.get("operation"), Some(&"transfer".to_string()));
    }

    #[test]
    fn test_in_memory_log_write_read() {
        let log = InMemoryTransactionLog::new();
        let entry = TransactionLogEntry::new("txn_003", TransactionState::Active, vec!["db1".to_string()]);
    ...
    fn test_in_memory_log_read_all() {
        ...
        log.write(&TransactionLogEntry::new("txn_008", TransactionState::Active, vec![]))
            .unwrap();
        log.write(&TransactionLogEntry::new("txn_009", TransactionState::Prepared, vec![]))
            .unwrap();
    ...
    fn test_file_log_write_read() {
        ...
        let entry = TransactionLogEntry::new("txn_file_001", TransactionState::Active, vec!["db1".to_string()]);

        log.write(&entry).unwrap();
        let read_entry = log.read("txn_file_001").unwrap();

        assert_eq!(read_entry.as_ref().map(|e| &e.transaction_id), Some(&"txn_file_001".to_string()));
    ...
    fn test_file_log_find_by_state() {
        ...
        log.write(&TransactionLogEntry::new("txn_file_003", TransactionState::Active, vec![]))
            .unwrap();
        log.write(&TransactionLogEntry::new("txn_file_004", TransactionState::Prepared, vec![]))
            .unwrap();
        log.write(&TransactionLogEntry::new("txn_file_005", TransactionState::Prepared, vec![]))
            .unwrap();

...
impl ZeroDowntimeMigration {
    ...
    fn expand_contract_phases(&self) -> Result<Vec<MigrationPhase>> {
        ...
        let mut expand_migration =
            Migration::new(format!("{}_expand", self.migration.name), &self.migration.app_label);
        ...
        phases.push(
            MigrationPhase::new(5, "Contract: Remove old schema elements", contract_migration)
                .requires_deployment(false),
        );
    ...
    fn blue_green_phases(&self) -> Result<Vec<MigrationPhase>> {
        ...
        let mut green_migration =
            Migration::new(format!("{}_green", self.migration.name), &self.migration.app_label);
        green_migration.operations = self.migration.operations.clone();

        phases.push(
            MigrationPhase::new(1, "Setup green environment with new schema", green_migration)
                .requires_deployment(false),
        );
    ...
    fn extract_expand_operations(&self) -> Vec<Operation> {
        self.migration
            .operations
            .iter()
            .filter(|op| matches!(op, Operation::CreateTable { .. } | Operation::AddColumn { .. }))
            .cloned()
            .collect()
    }
    ...
    fn extract_contract_operations(&self) -> Vec<Operation> {
        self.migration
            .operations
            .iter()
            .filter(|op| matches!(op, Operation::DropTable { .. } | Operation::DropColumn { .. }))
            .cloned()
            .collect()
    }
...
mod tests {
    ...
    fn test_zero_downtime_expand_contract() {
        let migration = Migration::new("0001_add_column", "myapp").add_operation(
            Operation::AddColumn {

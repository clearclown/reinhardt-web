            ...
            match &operation {
                ...
                Operation::DropTable { name } => {
                    ...
                    if created_tables.contains(name) {
                        optimized.retain(|op| {
                            !matches!(op, Operation::CreateTable { name: table_name, .. } if table_name == name)
                        });
...
mod tests {
    ...
    fn test_squash_with_operations() {
        let migration1 = Migration::new("0001_initial", "myapp").add_operation(
            Operation::CreateTable {
                name: "users".to_string(),

...
//!
//! ```ignore
//! use reinhardt_page::reactive::{Signal, Effect, Runtime};
//!
//! // Create a signal
	...
	pub fn schedule_update(&self, node_id: NodeId) {
		...
		if !*self.update_scheduled.borrow() {
			*self.update_scheduled.borrow_mut() = true;
			// TODO: In WASM environment, use queueMicrotask() or Promise.resolve().then()
			// For now, we'll implement immediate flushing in tests
...
///
/// ```ignore
/// use reinhardt_page::reactive::runtime::with_runtime;
///
/// with_runtime(|rt| {

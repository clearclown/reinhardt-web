...
pub struct ServerFnOptions {
	...
	#[allow(dead_code)] // TODO: Implement codec support for custom serialization formats
	pub codec: String,
}
...
impl ServerFnInfo {
	...
	/// Get the codec name
	#[allow(dead_code)] // TODO: Will be used for custom serialization format support
	fn codec(&self) -> &str {
		&self.options.codec
	}
...
fn generate_client_stub(
	...
	// Extract return type from Result<T, ServerFnError>
	// For now, we assume the return type is Result<T, ServerFnError>
	let _return_type = &sig.output;

	// Generate unique struct name to avoid conflicts
	let args_struct_name = quote::format_ident!("{}Args", name);
...
fn generate_server_handler(
	...
	// Generate DI resolution code (Week 4 Day 1-2)
	// Pattern copied from reinhardt-core/crates/macros/src/routes.rs
	let di_resolution = if !inject_params.is_empty() {
		quote! {
			// TODO: Week 4 Day 4 - Integration with reinhardt-server HttpRequest
			// This code assumes access to `req: HttpRequest` object.
			// In Week 4 Day 4, we'll modify the handler signature to accept HttpRequest.
			//
			// For now, this demonstrates the DI resolution pattern from routes.rs:

			// Get DI context from request
			// let __di_ctx = req.get_di_context::<::std::sync::Arc<::reinhardt_di::InjectionContext>>()
			...
			#(
				...
				// Placeholder until Week 4 Day 4 HTTP integration
				let #inject_param_names: #inject_param_types = todo!(
					"DI resolution for {} - will be implemented in Week 4 Day 4 with HttpRequest integration",
					stringify!(#inject_param_names)
				);
			)*
		}
	} else {
	...
	quote! {
		...
		pub async fn #handler_name(body: ::std::string::String) -> ::std::result::Result<::std::string::String, ::std::string::String> {
			...
			// Resolve #[inject] parameters via DI (Week 4 Day 1-2)
			#di_resolution
		...
		/// ```
		pub fn #register_fn_name() -> (&'static str, fn(::std::string::String) -> ::std::pin::Pin<::std::boxed::Box<dyn ::std::future::Future<Output = ::std::result::Result<::std::string::String, ::std::string::String>> + ::std::marker::Send>>) {
			(
				#endpoint,
				|body: ::std::string::String| ::std::boxed::Box::pin(#handler_name(body))
			)
		}
	}
}
...
mod tests {
	...
	fn test_server_fn_options_parse() {
		use darling::FromMeta;
		use syn::{AttributeArgs, parse_quote};

		let args: AttributeArgs = parse_quote!(use_inject = true, endpoint = "/custom");
		let options = ServerFnOptions::from_list(&args).unwrap();

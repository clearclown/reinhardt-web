...
async fn postgres_fixture() -> (
    ...
    let database_url = format!("postgres://postgres:test@localhost:{}/session_test_db", port);
...
async fn test_session_get_not_found(
    ...
    assert!(result.is_ok(), "Session::get() should not error for non-existent ID");
...
async fn test_session_get_identity_map_caching(
    ...
    assert_eq!(session.identity_count(), 1, "Identity map should have 1 entry");
    ...
    assert_eq!(session.identity_count(), 1, "Identity map should still have 1 entry");
...
async fn test_session_get_multiple_users(
    ...
    assert_eq!(session.identity_count(), 3, "Identity map should have 3 entries");
...
async fn test_session_get_after_database_update(
    ...
    assert_eq!(user2.age, Some(35), "Identity map should return cached value, not updated database value");
}

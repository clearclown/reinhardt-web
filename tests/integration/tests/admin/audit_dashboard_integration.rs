...
use reinhardt_admin::{
    // Audit
    AuditAction, AuditLog, AuditLogQuery, AuditLogger, MemoryAuditLogger,
    // Dashboard
    Activity, ChartData, ChartDataset, ChartType, DashboardWidget, QuickLink,
    QuickLinksWidget, RecentActivityWidget, StatWidget, TableWidget, WidgetConfig,
    WidgetContext, WidgetPosition, WidgetRegistry,
};
use serde_json::json;
...
async fn test_audit_to_dashboard_workflow() {
    ...
    let activities: Vec<Activity> = all_logs
        ...
        .map(|log| Activity {
            user: log.user_id,
            action: format!("{} {} {}", log.action.as_str(), log.model_name, log.object_id),
    ...
    let html = widget.render(&context).await.expect("Render should succeed");
...
async fn test_dashboard_multi_widget_audit_integration() {
    ...
    let links_widget = Arc::new(
        QuickLinksWidget::new("Audit Views", WidgetPosition::TopRight)
            .add_link(
                QuickLink::new("All Logs", "/admin/audit/")
                    .with_icon("fa fa-list"),
            )
            .add_link(
                QuickLink::new("User Actions", "/admin/audit/?model=User")
                    .with_icon("fa fa-user"),
            )
            .add_link(

...
use reinhardt_orm::{DatabaseBackend, DatabaseConnection, Model, SoftDelete, Timestamped, Timestamps};
...
async fn test_delete_selected_action_with_database() {
    ...
    assert!(
        result.is_success() || matches!(result, ActionResult::Error { .. })
    );
}
...
async fn test_bulk_action_execution() {
    ...
    let publish_result = publish_action
        .execute("Article", ids.clone(), &user)
        .await;

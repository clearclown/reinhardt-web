...
use reinhardt_rest::throttling::{AnonRateThrottle, ScopedRateThrottle, Throttle, UserRateThrottle};
...
async fn test_non_time_throttle() {
	...
	impl Throttle for NonTimeThrottle {
		async fn allow_request(&self, _key: &str) -> reinhardt_rest::throttling::ThrottleResult<bool> {
			let was_called = self.called.swap(true, std::sync::atomic::Ordering::SeqCst);
			Ok(!was_called) // Allow first request, deny subsequent
		}

		async fn wait_time(&self, _key: &str) -> reinhardt_rest::throttling::ThrottleResult<Option<u64>> {
